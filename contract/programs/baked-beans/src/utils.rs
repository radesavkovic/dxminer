...
pub fn calculate_eggs_buy(global_state: &GlobalState, my_sol: u64, total_sol: u64) -> Result<u64> {
    calculate_trade(
        global_state,
        my_sol as u128,
        total_sol as u128,
        global_state.market_eggs as u128,
    )
}

pub fn calculate_eggs_sell(
    global_state: &GlobalState,
    my_eggs: u64,
    total_sol_amt: u64,
) -> Result<u64> {
    msg!("my eggs {}", my_eggs);
    msg!("global_state.market_eggs {}", global_state.market_eggs);
...
pub fn buy_fee(amount: u64) -> Result<u64> {
    let res = (amount as u128) * 5 / 100;
    Ok(res as u64)
}

pub fn sell_fee(amount: u64) -> Result<u64> {
    let res = (amount as u128) * 5 / 100;
    Ok(res as u64)
}

pub fn get_eggs_since_last_hatch(
    ...
    let mut seconds_passed = cur_timestamp
        ...
        .unwrap();
    if seconds_passed > eggs_per_miner {
        seconds_passed = eggs_per_miner;
    }
